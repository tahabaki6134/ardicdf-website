...
(async()=>{try{await ready;for(const [url,lang,status] of [['/','en',200],['/tr','tr',200],['/contact','tr',200],['/en','en',200],['/ar','ar',200],['/ar/contact','ar',200],['/de/manufacturing/carbon-fiber','de',200],...['tr','en','de','fr','ru','ar'].map(lang=>[(lang==='tr'?'':'/'+lang)+'/unknown-test',lang,404])]){const r=await fetch(origin+url),html=await r.text();assert.equal(r.status,status,url);console.log(url,r.status,(html.match(/<html[^>]*>/)||[])[0]);assert.ok(html.includes('lang="'+lang+'"'),url);if(lang==='ar')assert.ok(html.includes('dir="rtl"'),url);if(status===404)assert.match(r.headers.get('x-robots-tag'),/noindex/);}
for(const [from,to] of [['/?utm_source=event','/en?utm_source=event'],['/live','/fabrication'],['/en/live','/en/fabrication'],['/compare?left=carbon&right=epoxy','/en/compare?left=carbon&right=epoxy'],['/services/large-format-3d-printing','/en/manufacturing/3d-printing']]){const r=await fetch(origin+from,{redirect:'manual'});assert.equal(r.status,308,from);assert.equal(new URL(r.headers.get('location'),origin).pathname+new URL(r.headers.get('location'),origin).search,to,from);}
const robots=await (await fetch(origin+'/robots.txt')).text();assert.ok(!robots.includes('Disallow: /review.html'));assert.match(robots,/Disallow: \/api\//);
const r=await fetch(origin+'/review.html');assert.equal(r.status,200);assert.match(r.headers.get('x-robots-tag'),/noindex/);console.log('HTTP status, legacy redirects and crawlable noindex checks passed');
