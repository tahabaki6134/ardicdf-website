...
(async()=>{try{await ready;for(const [url,lang,status] of [['/','tr',200],['/en','en',200],['/ar','ar',200],['/ar/contact','ar',200],['/de/manufacturing/carbon-fiber','de',200],['/ar/unknown-test','ar',404]]){const r=await fetch(origin+url),html=await r.text();assert.equal(r.status,status,url);console.log(url,r.status,(html.match(/<html[^>]*>/)||[])[0]);if(status===200)assert.ok(html.includes('lang="'+lang+'"'),url);}
