...
export function pageMetadata(locale: Locale, page: Page): Metadata {
  ...
  return { title: `${title} | ARDIÇ`, description, alternates: { canonical: url, languages }, robots: process.env.VERCEL_ENV === "preview" || page.kind === "selection" ? { index: false, follow: page.kind === "selection" } : { index: true, follow: true }, openGraph: { type: "website", title: `${title} | ARDIÇ`, description, url, locale: graphLocales[locale], siteName: "ARDIÇ Design & Fabrication", images: [{ url: `${siteOrigin}/og-image.png`, width: 1200, height: 630, alt: "ARDIÇ Design & Fabrication" }] }, twitter: { card: "summary_large_image", title: `${title} | ARDIÇ`, description, images: [`${siteOrigin}/og-image.png`] } };
}
