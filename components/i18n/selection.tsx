...
export function SelectionList({ locale, t, names }: { locale: Locale; t: Dictionary; names: Record<string, string> }) {
  const { selected, toggle, setSelected } = useProjectSelection();
  useEffect(() => { const ids = parseSelectedProjects(new URLSearchParams(window.location.search).get("selected")); if (ids.length) setSelected(ids); }, [setSelected]);
